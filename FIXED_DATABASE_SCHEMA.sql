-- Tracy AI Fixed Database Schema
...
-- Users table (extends Supabase auth.users) - handle existing table
...
-- Add username column if it doesn't exist (safely)
DO $$ 
BEGIN 
...
-- Calendar events - safely add missing columns
...
-- Add missing columns to calendar_events safely
DO $$ 
BEGIN 
  -- Add all_day column
...
-- Fix attendees column type if it's currently TEXT[] 
DO $$ 
BEGIN 
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'calendar_events' AND column_name = 'attendees' AND data_type = 'ARRAY') THEN
...
-- Indexes for performance (create only if columns exist)
DO $$ 
BEGIN 
  -- Username index (only if username column exists)
...
-- Success message
SELECT 'Tracy AI database schema updated successfully! All missing columns have been added safely.' as result; 