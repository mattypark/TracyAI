...
export function EventEditModal({ event, onClose, onSave }: EventEditModalProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    flagId: userFlags.find(f => f.name.toLowerCase() === event.flag_name?.toLowerCase())?.id || "default-personal",
    flagColor: event.flag_color || "#3B82F6",
    notification: "10",
    repeat: "never",
    visibility: "default",
  })
